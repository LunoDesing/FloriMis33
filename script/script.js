...
function updateTimer() {
    const targetDate = new Date("september 08 2024 13:00:00").getTime();
...
const video = document.querySelector('.video__save');
const fullscreenOverlay = document.getElementById('fullscreenOverlay');
const closeVideoButton = document.getElementById('closeVideoButton');  // Nuevo nombre para la variable

// Al hacer clic en el video, mostrarlo en pantalla completa con fondo traslúcido
video.addEventListener('click', () => {
    fullscreenOverlay.style.display = 'flex';
    const overlayVideo = fullscreenOverlay.querySelector('video') || document.createElement('video');
    overlayVideo.src = video.src;  // Usa el mismo video
    overlayVideo.controls = true;  // Habilita controles en pantalla completa
    fullscreenOverlay.appendChild(overlayVideo);  // Asegura que el video esté en el overlay
    overlayVideo.play();
});

// Cerrar pantalla completa al hacer clic en "Cerrar"
closeVideoButton.addEventListener('click', () => {
    const overlayVideo = fullscreenOverlay.querySelector('video');
    if (overlayVideo) {
        overlayVideo.pause();
        overlayVideo.src = '';  // Limpiar la fuente del video
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const recipientNumber1 = '543815393734'; // Número para el primer botón
